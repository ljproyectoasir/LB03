...
ORIGENES=( /etc/rc.local /etc/init.d/ /etc/ha.d/ /etc/haproxy/ /etc/bind/ /var/cache/bind/ /etc/nagios-nrpe/ /etc/redis/ /etc/network/interfaces /etc/resolv.conf /var/log/ /root/scripts/ )

# Destinos del backup (array) dentro de /backups/daily/
DESTINOS=( rc.local servicios hearbeat haproxy bind bind-zone nrpe redis conf-ip resolver logs scripts )
